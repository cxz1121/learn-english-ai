<template>
    <div class="min-h-[60vh] bg-zinc-50/80">
        <div class="w-[1200px] mx-auto px-4 pt-12 pb-24">
            ...
            <el-tabs v-model="currentTab" type="card" @tab-change="getList">
                <el-tab-pane label="精选课程" name="list"></el-tab-pane>
                <el-tab-pane v-if="userStore.user?.id" label="我的课程" name="my"></el-tab-pane>
            </el-tabs>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <article v-for="item in list" :key="item.id"
                    ...
                    <div class="p-5 flex-1 flex flex-col">
                        ...
                        <button type="button" @click="openPay(item)" v-if="currentTab === 'list'"
                        ...
                        <button type="button" @click="learnCourse(item)" v-if="currentTab === 'my'"
                            class="mt-4 w-full py-2.5 rounded-xl text-sm font-medium text-indigo-600 border border-indigo-200 bg-white hover:bg-indigo-50 transition-colors cursor-pointer">
                            学习课程
                        </button>
                    </div>
        ...
        <Pay v-model="payVisible" :course="selectedCourse" />
    </div>
</template>
