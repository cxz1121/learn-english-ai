  ...
  <Teleport to="body">
    <Transition name="pay-fade">
      <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        <!-- 遮罩 -->
        <div class="absolute inset-0 bg-zinc-900/50 backdrop-blur-[0.5px]" aria-hidden="true" />
...
const isPay = ref(false); // 是否正在支付
const timeExpire = ref(0); // 支付剩余时间倒计时
...
const close = () => {
  ...
  isPay.value = false; // 关闭弹窗时重置支付状态
  timeExpire.value = 0; // 关闭弹窗时重置支付剩余时间
}
const onConfirm = async () => {
  ...
  if (res.code === 200) {
    ...
    isPay.value = true; // 支付中
    timeExpire.value = res.data.timeExpire; // 支付剩余时间倒计时
  } else {
    ElMessage.error(res.message); // 显示错误信息
    isPay.value = false; // 支付失败时重置支付状态
  }
}
</script>
