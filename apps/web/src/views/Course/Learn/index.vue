...
import { getWordList, saveWordMasterApi } from '@/apis/learn'
...
const { playAudio } = useAudio({}) // 播放音频
const isLoading = ref<boolean>(false)
const route = useRoute()
const courseId = route.params.courseId as string // 课程id
const title = route.params.title || '我的课程' // 课程名称
const inputRefs = useTemplateRef<HTMLInputElement[]>('inputRefs')
const wordList = ref<WordItem[]>([]) // 拼写练习单词列表 bar -> b a r
const list = ref<Word[]>([]) // 单词列表
const currentIndex = ref(0)
const currentWord = computed<Word | undefined>(() => list.value[currentIndex.value])
const isWordBlurred = ref<boolean>(true)
watch(currentWord, () => {
  const current = currentWord.value?.word || ''
  wordList.value = current.split('').map((char) => ({
    word: char,
    isTrue: undefined,
    input: ''
  }))
...
const pageNext = () => {
  // const someError = wordList.value.some((item) => !item.isTrue)
  // if (someError) {
  //   ElMessage.error('请先完成拼写练习')
  //   return
  // }
  currentIndex.value++
}
const onInput = (index: number) => {
  ...
  nextTick(() => {
    ...
    if (index < inputs.length - 1) {
      inputs[index + 1].focus() // 自动聚焦下一个输入框
    }
  })
}
const onKeyDown = (index: number, event: KeyboardEvent) => {
  if (event.key === 'Backspace') {
    ...
    nextTick(() => {
      const inputs = inputRefs.value as HTMLInputElement[]
      if (index > 0) {
        inputs[index - 1].focus() // 自动聚焦上一个输入框
      }
    })
  }
  if (event.key.length === 1 && !event.ctrlKey && !event.altKey && !event.metaKey) {
    ...
    if (current.input && index < wordList.value.length - 1) {
      ...
      wordList.value[index + 1].isTrue = event.key === wordList.value[index + 1].word
      nextTick(() => {
        const inputs = inputRefs.value as HTMLInputElement[]
        inputs[index + 1].focus() // 自动聚焦下一个输入框
      })
...
const saveWordMaster = async () => {
  const wordIds = list.value.map((item) => item.id) // 从单词列表中提取单词id 数组
